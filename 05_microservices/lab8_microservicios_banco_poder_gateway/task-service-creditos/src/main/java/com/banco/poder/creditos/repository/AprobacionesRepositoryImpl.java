...
public class AprobacionesRepositoryImpl extends SpringJdbcDao implements AprobacionesRepository {
	...
	private String qrySeletById = "SELECT * FROM credito_autorizaciones WHERE ID_CREDITO = ? ORDER BY fecha_confirmacion desc limit 1 ";
	private String qryDelete = "DELETE FROM credito_autorizaciones WHERE ID_CREDITO = ? ";
	private String qrySeletByIdAndStatus = "SELECT * FROM credito_autorizaciones WHERE ID_CREDITO = ? AND ESTATUS = ? ORDER BY fecha_confirmacion desc limit 1 ";
	...
	@Override
	public AprobacionDto obtenerByIdAndStatus(String idCredito, boolean status) {
		try {
			return jdbcTemplate.queryForObject(qrySeletByIdAndStatus, new Object[] { idCredito ,status}, new RowMapper<AprobacionDto>() {
				@Override
				public AprobacionDto mapRow(ResultSet rs, int rownumber) throws SQLException {
					AprobacionDto aprobacionDto = new AprobacionDto();
					aprobacionDto.setId(rs.getString("id"));
