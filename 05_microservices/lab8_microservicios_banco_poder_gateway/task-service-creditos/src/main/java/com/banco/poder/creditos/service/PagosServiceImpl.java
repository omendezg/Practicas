...
import com.banco.poder.creditos.excepciones.CreditosNoAutorizadosException;
...
public class PagosServiceImpl implements PagosService {
	...
	public PagosServiceImpl(PagosRepository pagosRepository,CreditosRepository creditosRepository,AprobacionesService aprobacionesService) {
	...
	public PagosDto guardar(PagosDto pagosDto) {
		...
		if(aprobacionesService.obtenerByIdAndStatus(validate.getIdCredito()) != null) {
			pagosDto.setFecha(new Date().toString());

			return pagosRepository.persistir(pagosDto, pagosDto.getIdCredito());	
		}else {
			throw new CreditosNoAutorizadosException("No es posible generar pagos de creditos no autorizado", validate.getIdCredito());
		}
	}
